...
descriptors.pairs <- read.csv("https://raw.githubusercontent.com/jeremyfprice/litreview/master/data/lit_review-descriptors-pairs-el.csv", header = TRUE)

row.names(descriptors.pairs) <- descriptors.pairs$item1
descriptors.pairs$item1 <- NULL
descriptors.pairs$X <- NULL
descriptors.pairs$reflection <- NULL
descriptors.pairs$field.experience.programs <- NULL
...
descriptors.graph  <- simplify(as.undirected(graph.adjacency(as.matrix(descriptors.pairs), weighted=TRUE)))

  descriptors.communities <- cluster_optimal(descriptors.graph)
  print(descriptors.communities$modularity)
